...
class USVehicleAccidentAnalysis:
    ...
    def get_top_vehicle_contributing_to_injuries(self, output_path, output_format):
        ...
        df_top_5_to_15 = df.limit(15).subtract(df.limit(4))
    ...
    def get_top_5_vehicle_brand(self, output_path, output_format):
        ...
        """
        top_25_state_list = [row[0] for row in self.df_units.filter(col("VEH_LIC_STATE_ID").cast("int").isNull()).filter(col('VEH_HNR_FL')=='Y').
...
if __name__ == '__main__':
    ...
    print("3. Result:", usvaa.get_state_with_highest_female_accident(output_file_paths.get(3),
    ...
    print("4. Result:", usvaa.get_top_vehicle_contributing_to_injuries(output_file_paths.get(4),
                                                                       file_format.get("Output")))

    # 5. For all the body styles involved in crashes, mention the top ethnic user group of each unique body style
    print("5. Result:")
    usvaa.get_top_ethnic_ug_crash_for_each_body_style(output_file_paths.get(5), file_format.get("Output"))
    ...
    print("6. Result:", usvaa.get_top_5_zip_codes_with_alcohols_as_cf_for_crash(output_file_paths.get(6),
                                                                                file_format.get("Output")))

    # 7. Count of Distinct Crash IDs where No Damaged Property was observed and Damage Level (VEH_DMAG_SCL~) is above 4
    # and car avails Insurance
    print("7. Result:", usvaa.get_crash_ids_with_no_damage(output_file_paths.get(7), file_format.get("Output")))
    ...
    print("8. Result:", usvaa.get_top_5_vehicle_brand(output_file_paths.get(8), file_format.get("Output")))
